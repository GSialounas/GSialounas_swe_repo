function res = WENO5resAdv1d_fdm_gs_system_just_for_rec(w,flux,dflux,S,dx,eig_Jf_1, eig_Jf_2,F1,F2)
% *************************************************************************
...
%     a=max(abs(dflux(w)));
% else
%     eig_max_1 = max(eig_Jf_1(w));
%     eig_max_2 = max(eig_Jf_2(w));
%     a= max(eig_max_1,eig_max_2);
% end
  a=max(max(abs(dflux(w))));
...
res = (hp-circshift(hp,[0,1])+hn-circshift(hn,[0,1]))/dx;% - S;